...
SomeCherries 是一个常驻桌面的 cc-switch Token 用量监视器。消费金额会变成一盘樱桃：随着 Token 消耗，樱桃被慢慢吃掉，并通过三色告警灯提示当前用量、近 30 分钟速度和今日消费。

SomeCherries 会统计 cc-switch 中记录的全部用量和计费数据。无论请求来自中转站 API 还是官方订阅，只要 cc-switch 已记录相应的用量，SomeCherries 就能将其纳入统计。
...
- 将消费金额映射为会逐渐被吃掉的樱桃盘
- 统计 cc-switch 记录的全部用量，包括中转站 API 和官方订阅
